...
This compiles `FastKeyboardMouse.ahk` into `FastKeyboardMouse.exe` using `C:\Program Files\AutoHotkey\Compiler\Ahk2Exe.exe` with the light icon.

## 6. Publish the Release

If `tools\publish_settings.ini` exists, upload the exe and release notes to FTP:
...
If `tools\publish_settings.ini` does not exist, skip this step. To set up publishing, copy `tools\publish_settings_example.ini` to `tools\publish_settings.ini` and fill in your FTP credentials, signing configuration, and release notes paths.
...
**Implementation plan:**
- Add an `Ahk2Exe` resource directive in `FastKeyboardMouse.ahk` to embed the `release_notes/` folder (or at minimum the current version's `en.json`) as a resource
