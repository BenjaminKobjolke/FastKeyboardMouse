...
```bash
powershell -Command "cd 'D:\GIT\BenjaminKobjolke\FastTools\FastKeyboardMouse\tools'; cmd /c '.\script.bat'"
```
...
```bash
# timeout: 300000
powershell -Command "cd 'D:\GIT\BenjaminKobjolke\FastTools\FastKeyboardMouse\tools'; cmd /c '.\publish_release.bat'"
```
...
| Script | Description |
|--------|-------------|
| `build.bat` | Compile `FastKeyboardMouse.ahk` into `FastKeyboardMouse.exe` |
...
```bash
powershell -ExecutionPolicy Bypass -File "D:\GIT\BenjaminKobjolke\FastTools\FastKeyboardMouse\tools\increment_version.ps1"
```
